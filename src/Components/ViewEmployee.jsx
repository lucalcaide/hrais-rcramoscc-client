...
const ViewEmployee = () => {
  ...
  const handleExportToExcel = () => {
    ...
    XLSX.writeFile(wb, `${employee && employee.emp_no}_employee_details.xlsx`);
  };
  ...
  const handleDeactivate = () => {
    if (window.confirm('Are you sure you want to deactivate this employee?')) {
      if (window.confirm("By deactivating an employee's account, the employee will become inactive. Are you sure you want to proceed?")) {
        axios.put(`https://hrais-rcramoscc-server.onrender.com/auth/deactivate_employee/${id}`)
          .then(response => {
            if (response.data.Status) {
              toast.success('Employee deactivated successfully.');
              setEmployee({ ...employee, employee_status: 'Inactive', term_date: new Date().toLocaleDateString('en-US') });
            } else {
              toast.error(`Error: ${response.data.Error}`);
            }
          })
  ...
  return (
    <div>
      ...
      <button className='btn btn-back-color rounded-pill btn-text ms-2' onClick={handleDeactivate}>
        Deactivate Employee
      </button>
