...
const FilesApplicationForm = () => {
  ...
  const handleDelete = (id, emp_no) => {
    ...
    if (confirmDelete) {
      axios.delete(`https://hrais-rcramoscc-server.onrender.com/auth/delete_application_form/${id}`)
        .then(result => {
          if (result.data.Status) {
            // Update state first
  ...
  const handleFileChange = (id, emp_no, event) => {
    ...
    if (file) {
      ...
      axios.post(`https://hrais-rcramoscc-server.onrender.com/auth/upload_application_form/${id}`, formData, {
        headers: {
              ...
              <table className="table table-striped mt-3">
                <thead>
                  <tr>
                    ...
                    <th style={{ fontSize: '20px', fontWeight: 'bold', textAlign: 'left' }}>Application Form</th>
                ...
                <tbody>
                  {currentEmployees.map(e => (
                    <tr key={e.id}>
                      ...
                      <td style={{ fontSize: '20px', fontStyle: 'italic', textAlign: 'left', verticalAlign: 'middle' }}>{e.application_form}</td>
                      <td style={{ fontSize: '18px', textAlign: 'left', verticalAlign: 'middle' }}>
                        {e.application_form ? (
                          <a href={`https://hrais-rcramoscc-server.onrender.com/Public/ApplicationForms/` + e.application_form} className='btn btn-file-color btn-file-text rounded-pill' target="_blank" rel="noopener noreferrer">View Application Form</a>
                        ) : (
